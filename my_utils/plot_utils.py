# 修改了 dpvo.plot_utils 中的 save_ply 函数，修改成 save_ply_my_designed 函数
...
# DPVO相关导入
dpvo_dir = os.path.abspath(os.path.join(current_dir, "DPVO"))
...
def save_ply_my_designed(name: str, points: np.ndarray, colors: np.ndarray, output_dir: str):
    """
    保存点云数据到PLY文件，并存储到指定目录
    ...
    # 创建PLY元素
    el = PlyElement.describe(points_ply, 'vertex')
