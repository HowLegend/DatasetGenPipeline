# 修改了 Metric3D 中 mono.utils.do_test 中的函数（两个xxxx_my_designed）,其他函数照搬过来了
import logging
...
# Metric3D文件夹的路径
metric3d_path = os.path.join(project_root, 'Metric3D')
...
def do_scalecano_test_with_custom_data_my_designed(
    model: torch.nn.Module,
    cfg: dict,
    sdir,
    intrinsic_fx_fy,
    ...
    show_dir = sdir
    save_interval = 1
    save_imgs_dir = show_dir + '/depth_photo'
    os.makedirs(save_imgs_dir, exist_ok=True)
    save_pcd_dir = show_dir + '/depth_data'
    os.makedirs(save_pcd_dir, exist_ok=True)
    ...
    for i in tqdm(range(0, len(test_data), bs)):
        ...
        for an in batch_data:
            ...
            intrinsic = an['intrinsic']
            if intrinsic is None:
                intrinsic = [intrinsic_fx_fy[0], intrinsic_fx_fy[1], rgb_origin.shape[1]/2, rgb_origin.shape[0]/2]
        ...
        for j, gt_depth in enumerate(gt_depths):
            ...
            postprocess_per_image_my_designed(
                i*bs+j,
                pred_depths[j, :],
                gt_depth,
                intrinsic,
...
def postprocess_per_image_my_designed(i, pred_depth, gt_depth, intrinsic, rgb_origin, normal_out, pad, an, dam, dam_median, dam_global, is_distributed, save_imgs_dir, save_pcd_dir, normalize_scale, scale_info):
    ...
    if an['intrinsic'] == None:
        ...
        for r in [1.0]:
            ...
            for f in [1000]:
                pcd = reconstruct_pcd(pred_depth, f * r, f * (2-r), intrinsic[2], intrinsic[3])
                fstr = '_fx_' + str(int(f * r)) + '_fy_' + str(int(f * (2-r)))
                os.makedirs(osp.join(save_pcd_dir), exist_ok=True)
                # save_point_cloud(pcd.reshape((-1, 3)), rgb_origin.reshape(-1, 3), osp.join(save_pcd_dir, an['filename'][:-4] + fstr +'.ply'))
                save_point_cloud(pcd.reshape((-1, 3)), rgb_origin.reshape(-1, 3), osp.join(save_pcd_dir, "pointcloud_" + an['filename'][:-4] +'.ply'))
